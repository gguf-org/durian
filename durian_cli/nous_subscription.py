...
from durian_cli.auth import get_nous_auth_status
from durian_cli.config import get_env_value, load_config
...
def get_nous_subscription_explainer_lines() -> list[str]:
    ...
    return [
        ...
        "Change these later with: durian setup tools, durian setup terminal, or durian status.",
    ]


def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Nous defaults shared by `durian setup` and `durian model`."""
    if not managed_nous_tools_enabled():
        return set()
