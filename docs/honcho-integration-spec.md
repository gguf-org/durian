...
Comparison of Durian Agent vs. openclaw-honcho — and a porting spec for bringing Durian patterns into other Honcho integrations.

---

## Overview

Two independent Honcho integrations have been built for two different agent runtimes: **Durian Agent** (Python, baked into the runner) and **openclaw-honcho** (TypeScript plugin via hook/tool API). Both use the same Honcho peer paradigm — dual peer model, `session.context()`, `peer.chat()` — but they made different tradeoffs at every layer.

This document maps those tradeoffs and defines a porting spec: a set of Durian-originated patterns, each stated as an integration-agnostic interface, that any Honcho integration can adopt regardless of runtime or language.

> **Scope** Both integrations work correctly today. This spec is about the delta — patterns in Durian that are worth propagating and patterns in openclaw-honcho that Durian should eventually adopt. The spec is additive, not prescriptive.

---

## Architecture comparison

### Durian: baked-in runner
...
| Dimension | Durian Agent | openclaw-honcho |
|---|---|---|
...
| **AI peer name in identity** | Replaces "Durian Agent" in DEFAULT_AGENT_IDENTITY when configured. | Not implemented. |
...
Six patterns from Durian are worth adopting in any Honcho integration. Each is described as an integration-agnostic interface.
...
**openclaw-honcho contributes back (Durian should adopt):**
...
- [ ] **honcho mode / sessions / map** — CLI parity with Durian
...
Greenfield integration. Start from openclaw-honcho's architecture and apply all Durian patterns from day one.
