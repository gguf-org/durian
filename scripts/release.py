...
VERSION_FILE = REPO_ROOT / "durian_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "shannon@gguf-org.com": "shannonsands",
    ...
    "emozilla@gguf-org.com": "emozilla",
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "durian@gguf-org.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/gguf-org/durian",
    ...
    lines.append(f"# Durian Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Durian Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Durian Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Durian Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Durian Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Durian Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Durian Agent v{new_version} ({calver_date})' "
