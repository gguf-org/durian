"""
Multi-provider authentication system for Durian Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.durian/auth.json with cross-process file locking.
...
from durian_cli.config import get_durian_home, get_config_path, read_raw_config
...
DEFAULT_NOUS_PORTAL_URL = "https://portal.gguf-org.com"
DEFAULT_NOUS_INFERENCE_URL = "https://inference-api.gguf-org.com/v1"
DEFAULT_NOUS_CLIENT_ID = "durian-cli"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from durian_cli.copilot_auth import resolve_copilot_token
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `durian model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("DURIAN_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.durian/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    return get_durian_home() / "auth.json"
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Durian.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `durian logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from durian_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'durian doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'durian model' for available providers, or run 'durian doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'durian model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.durian/.env.",
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("DURIAN_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.durian/auth.json (not ~/.codex/)
#
# Durian maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Durian auth store (~/.durian/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `durian auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `durian auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `durian auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `durian auth` to re-authenticate.",
            provider="openai-codex",
...
def _write_codex_cli_tokens(
    ...
    *,
    ...
    When Durian refreshes a token it consumes the old refresh_token; if we
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Durian auth store (~/.durian/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Durian auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `durian auth` to re-authenticate."
            )
            relogin_required = True
        raise AuthError(
            message,
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Durian auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Durian's own Codex token store."""
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to Durian auth store")
            print("⚠️  Migrating Codex credentials to Durian's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `durian auth` to create a fully independent session.\n")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("DURIAN_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Durian processes
    ...
    base_url = (
        os.getenv("DURIAN_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    *,
    ...
    if effective_ca:
        ...
        if not os.path.isfile(ca_path):
            import logging
            logging.getLogger("durian.auth").warning(
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Durian models — they're not reliable for agentic tool-calling
            if "durian" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Durian is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("DURIAN_PORTAL_BASE_URL")
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "durian-cli"),
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Durian is not logged into Nous Portal.",
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `durian status` output.

    Checks the credential pool first (where the dashboard device-code flow
    and ``durian auth`` store credentials), then falls back to the legacy
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `durian auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `durian auth` and
    # `durian model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("DURIAN_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("DURIAN_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set DURIAN_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _prompt_model_selection(
    ...
    """
    from durian_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from durian_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'durian model' or 'durian setup' instead."""
    print("The 'durian login' command has been removed.")
    print("Use 'durian auth' to manage credentials,")
    print("'durian model' to select a provider, or 'durian setup' for full setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.durian/auth.json."""

    # Check for existing Durian-owned credentials
    try:
        ...
        if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
            print("Existing Codex credentials found in Durian auth store.")
            try:
    ...
    if cli_tokens:
        ...
        print("Durian will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            _save_codex_tokens(cli_tokens)
            base_url = os.getenv("DURIAN_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
            config_path = _update_config_for_provider("openai-codex", base_url)
            print()
            print("Credentials imported. Note: if Codex CLI refreshes its token,")
            print("Durian will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return

    # Run a fresh device code flow — Durian gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Durian creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Durian auth store
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Durian login via {pconfig.name}...")
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `durian model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("Durian will use OpenRouter for inference.")
        else:
            print("Run `durian model` or configure an API key to use Durian.")
    else:
