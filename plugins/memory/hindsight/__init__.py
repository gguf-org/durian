...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID   — memory bank identifier (default: durian)
  HINDSIGHT_BUDGET    — recall budget: low/mid/high (default: mid)
  HINDSIGHT_API_URL   — API endpoint
  HINDSIGHT_MODE      — cloud or local (default: cloud)

Or via $DURIAN_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $DURIAN_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_durian_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "durian": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "durian"),
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Durian Agent and the User"
    ...
    def save_config(self, values, durian_home):
        """Write config to $DURIAN_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(durian_home) / "hindsight"
    ...
    def post_setup(self, durian_home: str, config: dict) -> None:
        ...
        from durian_cli.memory_setup import _curses_select
        ...
        provider_config["bank_id"] = "durian"
        provider_config["recall_budget"] = "mid"
        bank_id = "durian"
        config["memory"]["provider"] = "hindsight"
        save_config(config)

        self.save_config(provider_config, durian_home)

        if env_writes:
            env_path = Path(durian_home) / ".env"
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name", "default": "durian"},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Durian Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "durian"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "durian"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = self._config.get("banks", {}).get("durian", {})
        self._bank_id = self._config.get("bank_id") or banks.get("bankId", "durian")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Durian Agent and the User")
