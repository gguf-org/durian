"""
Profile management for multiple isolated Durian instances.

Each profile is a fully independent DURIAN_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.durian/profiles/<name>/`` by default.

The "default" profile is ``~/.durian`` itself — backward compatible,
zero migration needed.

Usage::

    durian profile create coder          # fresh profile + bundled skills
    durian profile create coder --clone  # also copy config, .env, SOUL.md
    durian profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    durian -p coder chat                 # or via flag
    durian profile use coder             # set as sticky default
    durian profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See durian_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.durian) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "durian",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "durian", "default", "test", "tmp", "root", "sudo",
})

# Durian subcommands that cannot be used as profile names/aliases
_DURIAN_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the durian root, NOT to the current DURIAN_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where DURIAN_HOME points outside
    ``~/.durian``, profiles live under ``DURIAN_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_durian_home() / "profiles"


def _get_default_durian_home() -> Path:
    """Return the default (pre-profile) DURIAN_HOME path.

    In standard deployments this is ``~/.durian``.
    In Docker/custom deployments where DURIAN_HOME is outside ``~/.durian``
    (e.g. ``/opt/data``), returns DURIAN_HOME directly.
    """
    from durian_constants import get_default_durian_root
    return get_default_durian_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_durian_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.durian
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its DURIAN_HOME directory."""
    if name == "default":
        return _get_default_durian_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, durian subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _DURIAN_SUBCOMMANDS:
        return f"'{name}' conflicts with a durian subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_durian_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.durian)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches DURIAN_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "DURIAN_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.durian).\n"
            "To remove everything, use: durian uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set DURIAN_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("DURIAN_HOME")
    try:
        os.environ["DURIAN_HOME"] = str(profile_dir)
        from durian_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["DURIAN_HOME"] = old_home
        elif "DURIAN_HOME" in os.environ:
            del os.environ["DURIAN_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.durian/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: durian profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from DURIAN_HOME.

    Returns ``"default"`` if DURIAN_HOME is not set or points to ``~/.durian``.
    Returns the profile name if DURIAN_HOME points into ``~/.durian/profiles/<name>``.
    Returns ``"custom"`` if DURIAN_HOME is set to an unrecognized path.
    """
    from durian_constants import get_durian_home
    durian_home = get_durian_home()
    resolved = durian_home.resolve()

    default_resolved = _get_default_durian_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.durian itself — its parent is ~/ and its
        # directory name is ".durian", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: durian profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.durian itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.durian). "
            "Specify a different name: durian profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for durian profile names."""
    return '''# Durian Agent profile completion
# Add to ~/.bashrc: eval "$(durian completion bash)"

_durian_profiles() {
    local profiles_dir="$HOME/.durian/profiles"
...
_durian_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_durian_profiles)" -- "$cur"))
        return
    fi
...
complete -F _durian_completion durian
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for durian profile names."""
    return '''#compdef durian
# Durian Agent profile completion
# Add to ~/.zshrc: eval "$(durian completion zsh)"

_durian() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.durian/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.durian/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a DURIAN_HOME path string.

    Called early in the CLI entry point, before any durian modules
    are imported, to set the DURIAN_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: durian profile create {profile_name}"
        )
