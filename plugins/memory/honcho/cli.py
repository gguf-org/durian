...
Handles: durian honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "durian" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Durian profiles and creates host blocks for any that don't
    have one yet. Inherits settings from the default host block.
    """
    try:
        from durian_cli.profiles import list_profiles
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'durian honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'durian honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `durian update` -- no output, no exceptions.
    """
    try:
        from durian_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Durian profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $DURIAN_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_durian_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Durian persistent cross-session memory.")
    ...
    durian_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'durian honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = durian_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        durian_host["peerName"] = new_peer

    current_ai = durian_host.get("aiPeer") or cfg.get("aiPeer", "durian")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        durian_host["aiPeer"] = new_ai

    current_workspace = durian_host.get("workspace") or cfg.get("workspace", "durian")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        durian_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = durian_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in ("unified", "directional"):
        durian_host["observationMode"] = new_obs
    else:
        durian_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(durian_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        durian_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        durian_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # --- 6. Recall mode ---
    _raw_recall = durian_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        durian_host["recallMode"] = new_recall

    # --- 7. Session strategy ---
    current_strat = durian_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-directory")
    print("\n  Session strategy:")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        durian_host["sessionStrategy"] = new_strat

    durian_host["enabled"] = True
    durian_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        durian_config = load_config()
        durian_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(durian_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: durian config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    durian honcho status     -- show full config")
    print("    durian honcho mode       -- change recall/observation mode")
    print("    durian honcho tokens     -- tune context and dialectic budgets")
    print("    durian honcho peer       -- update peer names")
    print("    durian honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Durian profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: durian honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'durian honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: durian honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        durian = hosts.get(_host_key(), {})
        user = durian.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = durian.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = durian.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = durian.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Durian' identity in Honcho. Seed with 'durian honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: durian honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = durian.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = durian.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = durian.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Durian asks Honcho's AI peer a question")
        ...
        print("\n  Set with: durian honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'durian honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'durian honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    durian honcho identity --show        — show both peer representations")
        print("    durian honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Durian + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Durian\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Durian persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  durian honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'durian honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'durian honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  durian honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'durian'")
        print("  with Honcho configured and no prior session history.")
        print("  (Durian calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    durian honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'durian honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Durian, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'durian honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    durian honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  durian honcho identity <path/to/SOUL.md>")
    ...
    print("    Durian:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Durian:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Durian:   Honcho observes every message and updates representations")
    ...
    print("    Durian:   per-session by default — each run gets its own session")
    print("              Map a custom name:  durian honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. durian honcho setup              — configure API key (required)")
        print("  2. durian honcho migrate            — re-run this walkthrough")
    else:
        print("  1. durian honcho status             — verify Honcho connection")
        print("  2. durian                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. durian honcho identity --show    — verify AI peer representation")
        print("  4. durian honcho tokens             — tune context and dialectic budgets")
        print("  5. durian honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'durian memory setup'...\n")
        from durian_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``durian honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``durian honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to durian memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Durian Honcho",
    )
