...
```bash
durian memory setup    # select "supermemory"
```

Or manually:

```bash
durian config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.durian/.env
```

## Config

Config file: `$DURIAN_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `durian` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `durian-{identity}` → `durian-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Durian profile:

```json
{
  "container_tag": "durian-{identity}"
}
```

For a profile named `coder`, this resolves to `durian-coder`. The default profile resolves to `durian-default`. Without `{identity}`, all profiles share the same container.
