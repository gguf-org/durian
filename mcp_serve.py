"""
Durian MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Durian-specific extra)

Usage:
    durian mcp serve
    durian mcp serve --verbose
...
logger = logging.getLogger("durian.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using DURIAN_HOME."""
    try:
        from durian_constants import get_durian_home
        return get_durian_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("DURIAN_HOME", Path.home() / ".durian")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from durian_constants import get_durian_home
        directory_file = get_durian_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("DURIAN_HOME", Path.home() / ".durian")
...
class EventBridge:
    ...
    This is the Durian equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from durian_constants import get_durian_home
            db_file = get_durian_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("DURIAN_HOME", Path.home() / ".durian")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Durian MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'durian[mcp]'"
        )

    mcp = FastMCP(
        "durian",
        instructions=(
            "Durian Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Durian MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
