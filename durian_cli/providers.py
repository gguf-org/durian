"""
Single source of truth for provider identity in Durian Agent.
...
2. **Durian overlays** — transport type, auth patterns, aggregator flags,
...
# -- Durian overlay ----------------------------------------------------------
# Durian-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class DurianOverlay:
    """Durian-specific provider metadata layered on top of models.dev."""
...
DURIAN_OVERLAYS: Dict[str, DurianOverlay] = {
    "openrouter": DurianOverlay(
    ...
    "nous": DurianOverlay(
        ...
        base_url_override="https://inference-api.gguf-org.com/v1",
    ),
    "openai-codex": DurianOverlay(
    ...
    "qwen-oauth": DurianOverlay(
        ...
        base_url_env_var="DURIAN_QWEN_BASE_URL",
    ),
    "copilot-acp": DurianOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": DurianOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": DurianOverlay(
    ...
    "kimi-for-coding": DurianOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "minimax": DurianOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": DurianOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": DurianOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": DurianOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "vercel": DurianOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": DurianOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": DurianOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "durian", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Durian overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Durian overlay
    ...
    overlay = DURIAN_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + durian extra
    ...
    if overlay is not None:
        # Durian-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
