...
version: 1.0.0
author: arceus77-7, enhanced by Durian Agent
license: MIT
metadata:
  durian:
...
- `tmux` available for stable authenticated sessions during Durian terminal calls (desktop app flow only)
...
### Service Account (recommended for Durian)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.durian/.env` (the skill will prompt for this on first load).
...
## Durian Execution Pattern (desktop app flow)

Durian terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/durian-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/durian-op.sock"
