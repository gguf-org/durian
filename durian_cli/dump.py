"""
Dump command for durian CLI.

Outputs a compact, plain-text summary of the user's Durian setup
...
from durian_cli.config import get_durian_home, get_env_path, get_project_root, load_config
...
def _count_skills(durian_home: Path) -> int:
...
def _cron_summary(durian_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = durian_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"durian_home:      {display_durian_home()}")
    ...
    toolsets = config.get("toolsets", ["durian-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(durian_home)}")
    lines.append(f"  skills:             {_count_skills(durian_home)}")
