...
# Shimmer palette: 12 brightness stops from dim → bright → dim (neutral gray-durian)
# Used by KawaiiSpinner (raw terminal) and the TUI spinner widget (cli.py).
_SHIMMER_COLORS = (
    "#3a3a3a",
    "#4e4e4e",
    "#646464",
...
_SHIMMER_ANSI_CACHE = tuple(_hex_to_ansi_fg(c) for c in _SHIMMER_COLORS)


def _shimmer_ansi(text: str, t: float) -> str:
    """Return *text* with per-character ANSI 24-bit color forming a shimmer wave."""
    n = len(_SHIMMER_COLORS)
    speed = 1.5   # wave cycles per second
    spread = 0.20  # phase shift per character
    parts: list[str] = []
    for i, ch in enumerate(text):
        phase = (t * speed - i * spread) % 1.0
        idx = int(phase * n) % n
        parts.append(f"{_SHIMMER_ANSI_CACHE[idx]}{ch}")
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in Durian' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("DURIAN_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
            t = time.time()
