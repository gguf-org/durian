#!/usr/bin/env python3
"""
Durian CLI - Main entry point.

Usage:
    durian                     # Interactive chat (default)
    durian chat                # Interactive chat
    durian gateway             # Run gateway in foreground
    durian gateway start       # Start gateway as service
    durian gateway stop        # Stop gateway service
    durian gateway status      # Show gateway status
    durian gateway install     # Install gateway service
    durian gateway uninstall   # Uninstall gateway service
    durian setup               # Interactive setup wizard
    durian logout              # Clear stored authentication
    durian status              # Show status of all components
    durian cron                # Manage cron jobs
    durian cron list           # List cron jobs
    durian cron status         # Check if cron scheduler is running
    durian doctor              # Check configuration and dependencies
    durian honcho setup                    # Configure Honcho AI memory integration
    durian honcho status                   # Show Honcho config and connection status
    durian honcho sessions                 # List directory → session name mappings
    durian honcho map <name>               # Map current directory to a session name
    durian honcho peer                     # Show peer names and dialectic settings
    durian honcho peer --user NAME         # Set user peer name
    durian honcho peer --ai NAME           # Set AI peer name
    durian honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    durian honcho mode                     # Show current memory mode
    durian honcho mode [hybrid|honcho|local]  # Set memory mode
    durian honcho tokens                   # Show token budget settings
    durian honcho tokens --context N       # Set session.context() token cap
    durian honcho tokens --dialectic N     # Set dialectic result char cap
    durian honcho identity                 # Show AI peer identity representation
    durian honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    durian honcho migrate                  # Step-by-step migration guide: OpenClaw native → Durian + Honcho
    durian version             Show version
    durian update              Update to latest version
    durian uninstall           Uninstall Durian Agent
    durian acp                 Run as an ACP server for editor integration
    durian sessions browse     Interactive session picker with search

    durian claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (durian tools, durian setup, durian model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'durian {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any durian module import.
#
# Many modules cache DURIAN_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("DURIAN_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.durian/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set DURIAN_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check active_profile in the durian root
    if profile_name is None:
        try:
            from durian_constants import get_default_durian_root
            active_path = get_default_durian_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set DURIAN_HOME
    if profile_name is not None:
        try:
            from durian_cli.profiles import resolve_profile_env
            durian_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent durian from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["DURIAN_HOME"] = durian_home
...
# Load .env from ~/.durian/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from durian_cli.config import get_durian_home
from durian_cli.env_loader import load_durian_dotenv
load_durian_dotenv(project_env=PROJECT_ROOT / '.env')

# Initialize centralized file logging early — all `durian` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from durian_logging import setup_logging as _setup_logging
...
try:
    from durian_cli.config import load_config as _load_config_early
    from durian_constants import apply_ipv4_preference as _apply_ipv4
...
from durian_cli import __version__, __release_date__
from durian_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from durian_cli.config import get_env_path, get_durian_home, load_config
    from durian_cli.auth import get_auth_status

    # Determine whether Durian itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from durian_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_durian_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_durian_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Durian has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Durian to use their tokens.
    if _has_durian_config:
        try:
...
def _resolve_last_cli_session() -> Optional[str]:
    ...
    try:
        from durian_state import SessionDB
        db = SessionDB()
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, durian_bin
        cli_args: the original CLI arguments (everything after 'durian')
    """
    import shutil
    ...
    durian_bin = container_info["durian_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f'  }}];\n'
                    f"\n"
                    f"Or run: sudo durian {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo durian {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'durian sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Durian isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  durian setup")
        print()

        from durian_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'durian setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from durian_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["DURIAN_YOLO_MODE"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["DURIAN_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from durian_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Durian?")
        print()
    ...
    session_dir = get_durian_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: durian gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        print("⚠ Pairing may not have completed. Run 'durian whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from durian_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``durian model``) and the setup wizard
    ...
    from durian_cli.config import get_compatible_custom_providers, load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("DURIAN_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
    ...
    from durian_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.durian/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.durian/.env if the active provider is not 'custom'.
    ...
    """
    from durian_cli.config import get_env_value, save_env_value, load_config
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from durian_cli.setup import _curses_prompt_choice
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from durian_cli.config import get_env_value, save_config, save_env_value
    from durian_cli.nous_subscription import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from durian_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `durian auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from durian_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from durian_cli.models import fetch_api_models
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from durian_cli.curses_ui import flush_stdin
        flush_stdin()
        print()
...
from durian_cli.models import _PROVIDER_MODELS
...
def _model_flow_copilot(config, current_model=""):
    ...
    from durian_cli.config import save_env_value, load_config, save_config
    from durian_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from durian_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from durian_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Durian turns to `copilot --acp`.")
    print("  Durian currently starts its own ACP subprocess for each request.")
    print("  Durian uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set DURIAN_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from durian_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from durian_cli.config import get_env_value, save_env_value, load_config, save_config
    from durian_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from durian_constants import display_durian_home as _dhh_fn
            print(f"    Durian will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from durian_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'durian model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Durian CLI with a provider."""
    from durian_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from durian_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from durian_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from durian_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from durian_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from durian_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from durian_cli.doctor import run_doctor
    run_doctor(args)


def cmd_dump(args):
    """Dump setup summary for support/debugging."""
    from durian_cli.dump import run_dump
    run_dump(args)


def cmd_debug(args):
    """Debug tools (share report, etc.)."""
    from durian_cli.debug import run_debug
    run_debug(args)


def cmd_config(args):
    """Configuration management."""
    from durian_cli.config import config_command
    config_command(args)


def cmd_backup(args):
    """Back up Durian home directory to a zip file."""
    if getattr(args, "quick", False):
        from durian_cli.backup import run_quick_backup
        run_quick_backup(args)
    else:
        from durian_cli.backup import run_backup
        run_backup(args)


def cmd_import(args):
    """Restore a Durian backup from a zip file."""
    from durian_cli.backup import run_import
    run_import(args)


def cmd_version(args):
    """Show version."""
    print(f"Durian Agent v{__version__} ({__release_date__})")
    ...
    try:
        from durian_cli.banner import check_for_updates
        from durian_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Durian Agent."""
    _require_tty("uninstall")
    from durian_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``durian update --gateway`` so interactive prompts (stash restore,
    config migration) are forwarded to the messenger instead of being silently
    skipped.
    """
    import json as _json
    import uuid as _uuid
    from durian_constants import get_durian_home

    home = get_durian_home()
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``durian web``).
    ...
    if r1.returncode != 0:
        print(f"  {'✗' if fatal else '⚠'} Web UI npm install failed"
              + ("" if fatal else " (durian web will not be available)"))
        if fatal:
...
def _update_via_zip(args):
    """Update Durian Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/gguf-org/durian/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="durian-update-")
        zip_path = os.path.join(tmp_dir, f"durian-{branch}.zip")
        ...
        # GitHub ZIPs extract to durian-<branch>/
        extracted = os.path.join(tmp_dir, f"durian-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("durian-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Durian behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes durian completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Durian couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Durian couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Durian behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `durian update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/gguf-org/durian.git",
    "git@github.com:gguf-org/durian.git",
    "https://github.com/gguf-org/durian",
    "git@github.com:gguf-org/durian",
}
OFFICIAL_REPO_URL = "https://github.com/gguf-org/durian.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    from durian_constants import get_durian_home
    return (get_durian_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from durian_constants import get_durian_home
        (get_durian_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Durian repository.")
        print("  This means you may miss updates from gguf-org/durian.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/gguf-org/durian.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/gguf-org/durian.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``durian update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from durian_constants import get_default_durian_root
    default_home = get_default_durian_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "durian[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update Durian Agent to the latest version."""
    import shutil
    from durian_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Durian Agent")
        return
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/gguf-org/durian/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_durian_home added to durian_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload durian_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_durian_home() added since the last release.
        try:
            import importlib
            import durian_constants as _hc
        ...
        try:
            from durian_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'durian config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: durian config migrate")
            else:
                print()
                print("Skipped. Run 'durian config migrate' later to configure.")
        else:
        ...
        # When running as ``durian update --gateway`` (spawned by the gateway's
        # /update command), this process lives inside the gateway's systemd
        # cgroup.  ``systemctl restart durian-gateway`` kills everything in the
        ...
        if gateway_mode:
            _exit_code_path = get_durian_home() / ".update_exit_code"
            try:
        ...
        try:
            from durian_cli.gateway import (
            ...
            # Discover all durian-gateway* units (default + profiles)
            if supports_systemd_services():
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "durian-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. durian-gateway.service or durian-gateway-coder.service
            ...
            if is_macos():
                try:
                    from durian_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: durian gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print("    (or: durian -p <profile> gateway run  for each profile)")
        ...
        print("  durian model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``durian -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from durian_cli.profiles import (
    ...
    from durian_constants import display_durian_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `durian profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_durian_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → durian -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.durian)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  durian profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     durian -p {name} chat")
                else:
        ...
        name = args.profile_name
        from durian_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from durian_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from durian_cli.profiles import rename_profile
        try:
        ...
        from durian_cli.profiles import export_profile
        name = args.profile_name
        ...
        from durian_cli.profiles import import_profile
        try:
...
def cmd_dashboard(args):
    ...
    try:
        ...
        print("Install them with:  pip install durian[web]")
        sys.exit(1)

    if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
        sys.exit(1)

    from durian_cli.web_server import start_server
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from durian_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Durian log files."""
    from durian_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for durian CLI."""
    parser = argparse.ArgumentParser(
        prog="durian",
        description="Durian Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    durian                        Start interactive chat
    durian chat -q "Hello"        Single query mode
    durian -c                     Resume the most recent session
    durian -c "my project"        Resume a session by name (latest in lineage)
    durian --resume <session_id>  Resume a specific session by ID
    durian setup                  Run setup wizard
    durian logout                 Clear stored authentication
    durian auth add <provider>    Add a pooled credential
    durian auth list              List pooled credentials
    durian auth remove <p> <t>    Remove pooled credential by index, id, or label
    durian auth reset <provider>  Clear exhaustion status for a provider
    durian model                  Select default model
    durian config                 View configuration
    durian config edit            Edit config in $EDITOR
    durian config set model gpt-4 Set a config value
    durian gateway                Run messaging gateway
    durian -s durian-dev,github-auth
    durian -w                     Start in isolated git worktree
    durian gateway install        Install gateway background service
    durian sessions list          List past sessions
    durian sessions browse        Interactive session picker
    durian sessions rename ID T   Rename/title a session
    durian logs                   View agent.log (last 50 lines)
    durian logs -f                Follow agent.log in real time
    durian logs errors            View errors.log
    durian logs --since 1h        Lines from the last hour
    durian debug share             Upload debug report for support
    durian update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Durian Agent"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: durian-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Durian Agent with an interactive wizard. "
                    "Run a specific section: durian setup model|tts|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Durian CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: durian-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Durian Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Durian Agent setup"
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Durian setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Durian Agent. Use 'durian debug share' to "
                    "upload a debug report (system info + recent logs) to a paste "
                    "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    durian debug share              Upload debug report and print URL
    durian debug share --lines 500  Include more log lines
    durian debug share --expire 30  Keep paste for 30 days
    durian debug share --local      Print report locally (no upload)
""",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Durian home directory to a zip file",
        description="Create a zip archive of your entire Durian configuration, "
                    "skills, sessions, and data (excludes the durian codebase). "
                    "Use --quick for a fast snapshot of just critical state files."
    )
    backup_parser.add_argument(
        "-o", "--output",
        help="Output path for the zip file (default: ~/durian-backup-<timestamp>.zip)"
    )
    backup_parser.add_argument(
        "-q", "--quick",
        action="store_true",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Durian backup from a zip file",
        description="Extract a previously created Durian backup into your "
                    "Durian home directory, restoring configuration, skills, "
                    "sessions, and data"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Durian Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from durian_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from durian_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from durian_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/durian-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from durian_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from durian_cli.memory_setup import memory_command
            memory_command(args)
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'durian tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # durian tools list [--platform cli]
    ...
    # durian tools disable <name...> [--platform cli]
    ...
    # durian tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from durian_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from durian_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Durian as an MCP server",
        description=(
            "Manage MCP server connections and run Durian as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'durian mcp add' to connect to a new server, or\n"
            "'durian mcp serve' to expose Durian conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Durian as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from durian_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch durian --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            durian_bin = shutil.which("durian")
            if durian_bin:
                os.execvp(durian_bin, ["durian", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "durian_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Durian"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Durian",
    ...
    def cmd_claw(args):
        from durian_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Durian Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Durian Agent",
        description="Remove Durian Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Durian Agent as an ACP (Agent Client Protocol) server",
        description="Start Durian Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Durian Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Durian instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Durian Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Durian log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    durian logs                    Show last 50 lines of agent.log
    durian logs -f                 Follow agent.log in real time
    durian logs errors             Show last 50 lines of errors.log
    durian logs gateway -n 100     Show last 100 lines of gateway.log
    durian logs --level WARNING    Only show WARNING and above
    durian logs --session abc123   Filter by session ID
    durian logs --component tools  Only show tool-related lines
    durian logs --since 1h         Lines from the last hour
    durian logs --since 30m -f     Follow, starting from 30 min ago
    durian logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``durian -c Pokemon Agent Dev`` → ``durian -c 'Pokemon Agent Dev'``
    ...
    from durian_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'durian -c model' where 'model' is consumed as the
