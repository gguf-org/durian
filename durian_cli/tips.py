...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or durian -c.",
    ...
    "Ctrl+Z suspends Durian to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "durian -c resumes your most recent CLI session. durian -c \"project name\" resumes by title.",
    "durian -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "durian -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "durian chat -t web,terminal enables only specific toolsets for a focused session.",
    "durian chat -s github-pr-workflow preloads a skill at launch.",
    "durian chat -q \"query\" runs a single non-interactive query and exits.",
    "durian chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "durian chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "durian --yolo bypasses all dangerous command approval prompts for the entire session.",
    "durian chat --source telegram tags the session for filtering in durian sessions list.",
    "durian -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "durian doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "durian dump outputs a compact setup summary — great for bug reports.",
    "durian config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "durian config edit opens config.yaml in your default editor.",
    "durian config check scans for missing or stale configuration options.",
    "durian sessions browse opens an interactive session picker with search.",
    "durian sessions stats shows session counts by platform and database size.",
    "durian sessions prune --older-than 30 cleans up old sessions.",
    "durian skills search react --source skills-sh searches the skills.sh public directory.",
    "durian skills check scans installed hub skills for upstream updates.",
    "durian skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "durian skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "durian mcp add github --command npx adds MCP servers from the command line.",
    "durian mcp serve runs Durian itself as an MCP server for other agents.",
    "durian auth add lets you add multiple API keys for credential pool rotation.",
    "durian completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "durian logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "durian backup creates a zip backup of your entire Durian home directory.",
    "durian profile create coder creates an isolated profile that becomes its own command.",
    "durian profile create work --clone copies your current config and keys to a new profile.",
    "durian update syncs new bundled skills to ALL profiles automatically.",
    "durian gateway install sets up Durian as a system service (systemd/launchd).",
    "durian memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "durian webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as durian -w).",
    ...
    "DURIAN_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Durian tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'durian profile create coder' creates the 'coder' command.",
    "durian profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Durian prints a resume command with session ID and stats.",
    "durian sessions export backup.jsonl exports all sessions for backup or analysis.",
    "durian -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "durian skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .durian/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: durian cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Durian runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "durian gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Durian via a local macOS server.",
    ...
    "Context files (.durian.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.durian/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Durian loads project context from .durian.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — durian tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.durian/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "durian chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "durian plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "durian acp runs Durian as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "DURIAN_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "durian login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "durian status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.durian/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.durian/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Durian your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.durian/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Durian auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.durian/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at DURIAN_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "DURIAN_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in DURIAN_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.durian/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'durian profile create ls' would be rejected.",
    "durian profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "durian claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "DURIAN_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via durian tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.durian/.env.",
    "Skills from trusted repos (gguf-org) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.durian/skills/.hub/quarantine/ holds skills pending security review.",
]
