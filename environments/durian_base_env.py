"""
DurianAgentBaseEnv -- Abstract Base Environment for Durian + Atropos

Provides the Atropos integration plumbing that all durian environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via DurianAgentLoop
...
# Ensure the durian repo root is on sys.path so that imports like
...
# Load API keys from durian/.env so all environments can access them
...
from environments.agent_loop import AgentResult, DurianAgentLoop
...
# Import durian toolset infrastructure
...
class DurianAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for durian Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of durian toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: durian, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/durian-results/ "
...
class DurianAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for durian Atropos integration.
    ...
    name: Optional[str] = "durian"
    env_config_cls = DurianAgentEnvConfig

    def __init__(
        self,
        config: DurianAgentEnvConfig,
        ...
        # Set terminal environment variables so durian tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY durian tool (terminal, file, web,
