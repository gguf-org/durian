...
from durian_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``DURIAN_PLATFORM`` or
            ``DURIAN_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.durian/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.durian/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    durian = metadata.get("durian") or {}
    if not isinstance(durian, dict):
        durian = {}
    return {
        "fallback_for_toolsets": durian.get("fallback_for_toolsets", []),
        "requires_toolsets": durian.get("requires_toolsets", []),
        "fallback_for_tools": durian.get("fallback_for_tools", []),
        "requires_tools": durian.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    durian = metadata.get("durian")
    if not isinstance(durian, dict):
        return []
    raw = durian.get("config")
    if not raw:
        return []
