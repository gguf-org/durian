"""
Durian Plugin System
====================

Discovers, loads, and manages plugins from three sources:

1. **User plugins**   – ``~/.durian/plugins/<name>/``
2. **Project plugins** – ``./.durian/plugins/<name>/`` (opt-in via
   ``DURIAN_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``durian_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "durian_agent.plugins"

_NS_PARENT = "durian_plugins"
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``durian honcho ...``).
    ...
    def register_skill(
        ...
        ``~/.durian/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.durian/plugins/)
        user_dir = get_durian_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.durian/plugins/)
        if _env_enabled("DURIAN_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".durian" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``durian_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``durian tools`` TUI so plugin-provided toolsets appear
