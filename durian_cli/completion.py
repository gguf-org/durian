"""Shell completion script generation for durian CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_durian_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Durian Agent bash completion
# Add to ~/.bashrc:
#   eval "$(durian completion bash)"
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef durian
# Durian Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(durian completion zsh)"
...
_durian() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_durian_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'durian command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Durian Agent fish completion",
        "# Add to your config:",
        "#   durian completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __durian_profiles",
        "    echo default",
        "    if test -d $HOME/.durian/profiles",
        "        ls $HOME/.durian/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c durian -f -s p -l profile"
        " -d 'Profile name' -xa '(__durian_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__durian_profiles)' -d 'Profile name'"
                )
