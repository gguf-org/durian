---
name: honcho
description: Configure and use Honcho memory with Durian -- cross-session user modeling, multi-profile peer isolation, observation config, and dialectic reasoning. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation and recall settings.
version: 1.0.0
author: Durian Agent
license: MIT
metadata:
...
# Honcho Memory for Durian

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Durian profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/durian#running-honcho-locally-with-durian

### Verify

```bash
durian honcho status    # shows resolved config, connection test, peer info
```
...
Honcho models conversations as interactions between **peers**. Durian creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Durian instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Durian run |
| `global` | Single session across all directories |

Manual override: `durian honcho map my-project-name`
...
Each Durian profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
durian profile create coder --clone
# creates host block durian.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `durian.coder` host block in `honcho.json`
...
```bash
durian honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Config file: `$DURIAN_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `durian honcho setup`. Ensure `memory.provider: honcho` is in `~/.durian/config.yaml`.

### Memory not persisting across sessions
Check `durian honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `durian profile create <name> --clone`. For existing profiles: `durian honcho sync`.
...
| Command | Description |
|---------|-------------|
| `durian honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `durian honcho status` | Show resolved config, connection test, peer info for active profile |
| `durian honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `durian honcho disable` | Disable Honcho for the active profile |
| `durian honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `durian honcho peers` | Show peer identities across all profiles |
| `durian honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `durian honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `durian honcho sessions` | List known directory-to-session-name mappings |
| `durian honcho map <name>` | Map current working directory to a Honcho session name |
| `durian honcho identity` | Seed AI peer identity or show both peer representations |
| `durian honcho sync` | Create host blocks for all Durian profiles that don't have one yet |
| `durian honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Durian + Honcho |
| `durian memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `durian memory status` | Show active memory provider and config |
| `durian memory off` | Disable external memory provider |
