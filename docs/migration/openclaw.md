# Migrating from OpenClaw to Durian Agent

This guide covers how to import your OpenClaw settings, memories, skills, and API keys into Durian Agent.

## Three Ways to Migrate

### 1. Automatic (during first-time setup)

When you run `durian setup` for the first time and Durian detects `~/.openclaw`, it automatically offers to import your OpenClaw data before configuration begins. Just accept the prompt and everything is handled for you.

### 2. CLI Command (quick, scriptable)

```bash
durian claw migrate                      # Preview then migrate (always shows preview first)
durian claw migrate --dry-run            # Preview only, no changes
durian claw migrate --preset user-data   # Migrate without API keys/secrets
durian claw migrate --yes                # Skip confirmation prompt
```
...
```
> Migrate my OpenClaw setup to Durian
```
...
|------|--------|-------------|
| SOUL.md | `~/.openclaw/workspace/SOUL.md` | `~/.durian/SOUL.md` |
| Memory entries | `~/.openclaw/workspace/MEMORY.md` | `~/.durian/memories/MEMORY.md` |
| User profile | `~/.openclaw/workspace/USER.md` | `~/.durian/memories/USER.md` |
| Skills | `~/.openclaw/workspace/skills/` | `~/.durian/skills/openclaw-imports/` |
| Command allowlist | `~/.openclaw/workspace/exec_approval_patterns.yaml` | Merged into `~/.durian/config.yaml` |
| Messaging settings | `~/.openclaw/config.yaml` (TELEGRAM_ALLOWED_USERS, MESSAGING_CWD) | `~/.durian/.env` |
| TTS assets | `~/.openclaw/workspace/tts/` | `~/.durian/tts/` |
...
| Item | Source | Destination |
|------|--------|-------------|
| Telegram bot token | `openclaw.json` channels config | `~/.durian/.env` |
| OpenRouter API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.durian/.env` |
| OpenAI API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.durian/.env` |
| Anthropic API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.durian/.env` |
| ElevenLabs API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.durian/.env` |
...
- **TTS provider**: OpenClaw renamed "edge" to "microsoft" — both are recognized and mapped to Durian' "edge"
...
By default, the migration **will not overwrite** existing Durian data:

- **SOUL.md** — skipped if one already exists in `~/.durian/`
...
- **API keys** — skipped if the key is already set in `~/.durian/.env`
...
For executed migrations, the full report is saved to `~/.durian/migration/openclaw/<timestamp>/`.

## Post-Migration Notes

- **Skills require a new session** — imported skills take effect after restarting your agent or starting a new chat.
- **WhatsApp requires re-pairing** — WhatsApp uses QR-code pairing, not token-based auth. Run `durian whatsapp` to pair.
- **Archive cleanup** — after migration, you'll be offered to rename `~/.openclaw/` to `.openclaw.pre-migration/` to prevent state confusion. You can also run `durian claw cleanup` later.
...
```bash
durian claw migrate --source /path/to/.openclaw
```

### "Migration script not found"
The migration script ships with Durian Agent. If you installed via pip (not git clone), the `optional-skills/` directory may not be present. Install the skill from the Skills Hub:
```bash
durian skills install openclaw-migration
```

### Memory overflow
If your OpenClaw MEMORY.md or USER.md exceeds Durian' character limits, excess entries are exported to an overflow file in the migration report directory. You can manually review and add the most important ones.
...
The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `durian config set`.
