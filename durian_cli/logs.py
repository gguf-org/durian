"""``durian logs`` — view and filter Durian log files.
...
Usage examples::
    ...
    durian logs                    # last 50 lines of agent.log
    durian logs -f                 # follow agent.log in real time
    durian logs errors             # last 50 lines of errors.log
    durian logs gateway -n 100    # last 100 lines of gateway.log
    durian logs --level WARNING    # only WARNING+ lines
    durian logs --session abc123   # filter by session ID substring
    durian logs --component tools  # only tool-related lines
    durian logs --since 1h         # lines from the last hour
    durian logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_durian_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Durian runs — try 'durian chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_durian_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_durian_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_durian_home()}/logs/")
        return

    print(f"Log files in {display_durian_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'durian chat' to generate logs)")
