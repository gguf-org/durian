...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_durian_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:durian_pkce",
    ...
    raise SystemExit(f"`durian auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # If this was a singleton-seeded credential (OAuth device_code, durian_pkce),
    ...
    # re-seeded on the next load_pool() call.
    ...
    elif removed.source == "durian_pkce" and provider == "anthropic":
        from durian_constants import get_durian_home
        oauth_file = get_durian_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared Durian Anthropic OAuth credentials")

    elif removed.source == "claude_code" and provider == "anthropic":
        from durian_cli.auth import suppress_credential_source
        ...
        print("Run `durian auth add anthropic` to re-enable if needed.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `durian auth` is called bare."""
