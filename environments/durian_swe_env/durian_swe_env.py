"""
DurianSweEnv -- SWE-Bench Style Environment with Modal Sandboxes
...
Usage:
    ...
    vllm serve YourModel --tool-parser durian
    run-api
    python environments/durian_swe_env.py serve \\
...
class DurianSweEnv(DurianAgentBaseEnv):
    ...
    name = "durian-swe"
    env_config_cls = DurianSweEnvConfig

    @classmethod
    def config_init(cls) -> Tuple[DurianSweEnvConfig, List[APIServerConfig]]:
        """
        ...
        env_config = DurianSweEnvConfig(
            ...
            # Atropos settings
            group_size=4,
            tokenizer_name="gguf-org/DeepDurian-3-Llama-3-3B-Preview",
            tool_call_parser="durian",
            steps_per_eval=50,
            total_steps=500,
            use_wandb=True,
            wandb_name="durian-swe",
        )

        server_configs = [
            APIServerConfig(
                base_url="http://localhost:8000/v1",
                model_name="gguf-org/DeepDurian-3-Llama-3-3B-Preview",
