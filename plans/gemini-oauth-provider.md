...
## Token Lifecycle
- Store at `~/.durian/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `durian_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `durian_cli/models.py` — Add Gemini model catalog
4. `durian_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `durian_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `durian_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
- OR: accept user-provided client_id via DURIAN_GEMINI_CLIENT_ID env var
...
- durian Copilot OAuth: `durian_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
