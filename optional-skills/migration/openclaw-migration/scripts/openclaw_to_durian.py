#!/usr/bin/env python3
"""OpenClaw -> Durian migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Durian Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Durian.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Durian memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Durian memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Durian-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Durian-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Durian command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.durian/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.durian/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Durian .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Durian .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Durian .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Durian .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Durian .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Durian config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Durian config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Durian.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Durian MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Durian config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'durian cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Durian config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Durian session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Durian custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Durian config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Durian config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Durian config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Durian config.yaml")
    ensure_parent(path)
    path.write_text(
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Durian in migrated text so that
...
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Durian'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Durian'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Durian'),
]


def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Durian."""
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Durian Migration Report",
        "",
...
class Migrator:
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Durian config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Durian
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Durian-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Durian-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in ("file", "exec"):
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: durian config set",
                        )
                    continue
        ...
        for oc_key, durian_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and durian_key not in secret_additions:
                secret_additions[durian_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, durian_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and durian_key not in secret_additions:
                        secret_additions[durian_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        durian_config = load_yaml_file(destination)
        current_model = durian_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = durian_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                durian_config["model"] = {"default": model_str}
            dump_yaml_file(destination, durian_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in ("elevenlabs", "openai", "edge", "microsoft"):
            # OpenClaw renamed "edge" to "microsoft"; Durian still uses "edge"
        ...
        durian_config = load_yaml_file(destination)
        existing_tts = durian_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Durian destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Durian-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Durian-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        durian_cfg_path = self.target_root / "config.yaml"
        durian_cfg = load_yaml_file(durian_cfg_path)
        existing_mcp = durian_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Durian config")
                continue

            durian_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                durian_srv["command"] = srv["command"]
                if srv.get("args"):
                    durian_srv["args"] = srv["args"]
                if srv.get("env"):
                    durian_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    durian_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                durian_srv["url"] = srv["url"]
                if srv.get("headers"):
                    durian_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    durian_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                durian_srv["enabled"] = False
            if srv.get("timeout"):
                durian_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                durian_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                durian_srv["tools"] = {}
                if tools_cfg.get("include"):
                    durian_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    durian_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(durian_cfg_path)
            durian_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(durian_cfg_path, durian_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'durian cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = durian_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Durian reasoning_effort
        ...
        if compaction:
            compression = durian_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'durian gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("DURIAN_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        durian_cfg_path = self.target_root / "config.yaml"
        durian_cfg = load_yaml_file(durian_cfg_path)
        sr = durian_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        durian_cfg_path = self.target_root / "config.yaml"
        durian_cfg = load_yaml_file(durian_cfg_path)
        custom_providers = durian_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(durian_cfg_path)
            durian_cfg["custom_providers"] = custom_providers
            dump_yaml_file(durian_cfg_path, durian_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Durian config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            durian_cfg_path = self.target_root / "config.yaml"
            durian_cfg = load_yaml_file(durian_cfg_path)
            discord_durian = durian_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_durian["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_durian["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                durian_cfg["discord"] = discord_durian
                dump_yaml_file(durian_cfg_path, durian_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        durian_cfg_path = self.target_root / "config.yaml"
        durian_cfg = load_yaml_file(durian_cfg_path)
        browser_durian = durian_cfg.get("browser") or {}
        changed = False

        # Map fields that have Durian equivalents
        if browser.get("cdpUrl"):
            browser_durian["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_durian["headless"] = browser["headless"]
            changed = True

        if changed:
            durian_cfg["browser"] = browser_durian
            if self.execute:
                self.maybe_backup(durian_cfg_path)
                dump_yaml_file(durian_cfg_path, durian_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            durian_mode = mode_map.get(mode, "manual")
            durian_cfg.setdefault("approvals", {})["mode"] = durian_mode
            if self.execute:
                self.maybe_backup(durian_cfg_path)
                dump_yaml_file(durian_cfg_path, durian_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{durian_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Durian Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Durian. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Durian Config Not Overwritten)",
                "",
                "These items already existed in your Durian config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Durian agent discovers these",
            "directories, it may read/write to them instead of the Durian state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `durian claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `durian claw cleanup` to fix it.",
            "",
            "## Durian-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `durian claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `durian setup` to configure any remaining settings",
            "- Run `durian mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `durian cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `durian cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `durian gateway install` if you need the gateway service",
            "- Review `~/.durian/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Durian Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=str(Path.home() / ".durian"), help="Durian home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Durian targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Durian-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Durian Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    print(f"  ║  Source:  {str(report['source_root'])[:42]:<42s}  ║")
    print(f"  ║  Target:  {str(report['target_root'])[:42]:<42s}  ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    print(f"  ║  ✔ Migrated:  {s.get('migrated', 0):>3d}    𖤓 Archived:  {s.get('archived', 0):>3d}        ║")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.durian/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if archived:
        ...
        for item in archived:
            ...
            print(f"    𖤓 {display:<35s}  {short_reason}")
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.durian/config.yaml")
        print("    2. Run: durian mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: durian cron")
