...
```bash
durian memory setup    # select "holographic"
```

Or manually:
```bash
durian config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.durian-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$DURIAN_HOME/memory_store.db` | SQLite database path |
