"""Durian CLI skin/theme engine.

A data-driven skin system that lets users customize the CLI's visual appearance.
Skins are defined as YAML files in ~/.durian/skins/ or as built-in presets.
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Durian Agent"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! 𖤓"              # Shown on exit
      response_label: " 𖤓 Durian "       # Response box header label
...
.. code-block:: python
    ...
    from durian_cli.skin_engine import get_active_skin, list_skins, set_active_skin

    skin = get_active_skin()
    print(skin.colors["banner_title"])    # "#FFD700"
    print(skin.get_branding("agent_name"))  # "Durian Agent"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.durian/skins/
...
- ``warm-lightmode`` — Warm brown/durian text for light terminal backgrounds

USER SKINS
==========

Drop a YAML file in ``~/.durian/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces DURIAN_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces DURIAN_CADUCEUS)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Durian — durian and kawaii",
        "colors": {
...
[dim #6B1717]⠀⠀⠀⠀⠀⠀⠀⠀war god online⠀⠀⠀⠀⠀⠀⠀⠀[/]""",
    ...
    "warm-lightmode": {
        "name": "warm-lightmode",
        "description": "Warm light mode — dark brown/durian text for light terminal backgrounds",
        "colors": {
...
def get_active_goodbye(fallback: str = "Goodbye! 𖤓") -> str:
