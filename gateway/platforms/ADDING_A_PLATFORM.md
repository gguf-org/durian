...
Checklist for integrating a new messaging platform into the Durian gateway.
...
And add it to the `durian-gateway` composite:

```python
"durian-gateway": {
    "includes": [..., "durian-your-platform"]
}
```
...
## 12. Status Display (`durian_cli/status.py`)
...
## 13. Gateway Setup Wizard (`durian_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ durian_cli/ toolsets.py \
  --include="*.py" -l | sort -u
