...
- smoke-test a server with the FastMCP CLI before wiring it into Durian or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Durian. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.durian/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.durian/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Durian integration, either:

- configure the server in `~/.durian/config.yaml` using the `native-mcp` skill, or
...
### Durian cannot see the deployed server

The server-building part may be correct while the Durian config is not. Load the `native-mcp` skill and configure the server in `~/.durian/config.yaml`, then restart Durian.
