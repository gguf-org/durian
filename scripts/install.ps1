# ============================================================================
# Durian Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$DurianHome = "$env:LOCALAPPDATA\durian",
    [string]$InstallDir = "$env:LOCALAPPDATA\durian\durian"
)
...
$RepoUrlSsh = "git@github.com:gguf-org/durian.git"
$RepoUrlHttps = "https://github.com/gguf-org/durian.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Test-Node {
    ...
    $managedNode = "$DurianHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$DurianHome\node;$env:Path"
        Write-Success "Node.js $version found (Durian-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.durian/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\durian-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$DurianHome\node") { Remove-Item -Recurse -Force "$DurianHome\node" }
                Move-Item $extractedDir.FullName "$DurianHome\node"
                $env:Path = "$DurianHome\node;$env:Path"

                $version = & "$DurianHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.durian/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/gguf-org/durian/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\durian-$Branch.zip"
                $extractPath = "$env:TEMP\durian-extract"
...
function Install-Dependencies {
    ...
    # tinker-atropos (RL training backend) is optional — skip by default.
    # To enable RL tools: git submodule update --init tinker-atropos && uv pip install -e ".\tinker-atropos"
...
function Set-PathVariable {
    Write-Info "Setting up durian command..."
    
    if ($NoVenv) {
        $durianBin = "$InstallDir"
    } else {
        $durianBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so durian is globally available
    # On Windows, the durian.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$durianBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$durianBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $durianBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set DURIAN_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\durian instead
    # of the Unix default ~/.durian
    $currentDurianHome = [Environment]::GetEnvironmentVariable("DURIAN_HOME", "User")
    if (-not $currentDurianHome -or $currentDurianHome -ne $DurianHome) {
        [Environment]::SetEnvironmentVariable("DURIAN_HOME", $DurianHome, "User")
        Write-Success "Set DURIAN_HOME=$DurianHome"
    }
    $env:DURIAN_HOME = $DurianHome
    
    # Update current session
    $env:Path = "$durianBin;$env:Path"
    
    Write-Success "durian command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.durian directory structure
    New-Item -ItemType Directory -Force -Path "$DurianHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DurianHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DurianHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DurianHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DurianHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DurianHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DurianHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DurianHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DurianHome\skills" | Out-Null
    New-Item -ItemType Directory -Force -Path "$DurianHome\whatsapp\session" | Out-Null
    
    # Create .env
    $envPath = "$DurianHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.durian/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.durian/.env"
        }
    } else {
        Write-Info "~/.durian/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$DurianHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.durian/config.yaml from template"
        }
    } else {
        Write-Info "~/.durian/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$DurianHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.durian/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.durian/"
    
    # Seed bundled skills into ~/.durian/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.durian/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.durian/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$DurianHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.durian/skills/"
            }
        }
    }
}
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m durian_cli.main setup
    } else {
        python -m durian_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $durianCmd = "$InstallDir\venv\Scripts\durian.exe"
    if (-not (Test-Path $durianCmd)) {
        $durianCmd = "durian"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$DurianHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'durian whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$DurianHome\logs\gateway.log"
            Start-Process -FilePath $durianCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$DurianHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: durian gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: durian gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$DurianHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$DurianHome\cron\, sessions\, logs\"
    ...
    Write-Host "   durian setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   durian config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   durian config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   durian gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   durian update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/gguf-org/durian/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
