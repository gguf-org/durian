# Durian Atropos Environments

This directory contains the integration layer between **durian's** tool-calling capabilities and the **Atropos** RL training framework. It provides everything needed to run agentic LLMs through multi-turn tool-calling loops, score their output with arbitrary reward functions, and feed results into Atropos for training or evaluation.
...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  DurianAgentBaseEnv    │  durian_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
              ...
              ┌─────────────────┼─────────────────┐
              │                 │                  │
     TerminalTestEnv     DurianSweEnv    TerminalBench2EvalEnv
...
**DurianAgentBaseEnv** (`durian_base_env.py`) extends BaseEnv with durian specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, modal, daytona, ssh, singularity)
- Resolves durian toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `DurianAgentBaseEnv` and implement:
...
`DurianAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as durian's `run_agent.py`:
...
`ToolContext` is a per-rollout handle that gives reward/verification functions direct access to **all** durian tools, scoped to the rollout's `task_id`. The same `task_id` means the terminal/browser session is the SAME one the model used during its rollout -- all state (files, processes, browser tabs) is preserved.
...
- **Generic**: `call_tool(name, args)` -- call any durian tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some durian tools use `asyncio.run()` internally (e.g., the Modal backend). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Available parsers:
- `durian` -- Durian/ChatML `<tool_call>` XML format
...
├── durian_base_env.py            # Abstract base (DurianAgentBaseEnv)
├── agent_loop.py                 # Multi-turn agent engine (DurianAgentLoop)
...
│
├── durian_swe_env/               # SWE-bench style training environment
│   └── durian_swe_env.py
│
...
### DurianSweEnv (`durian_swe_env/`)

SWE-bench style training environment. The model gets a coding task, uses terminal + file + web tools to solve it, and the reward function runs tests in the same Modal sandbox.

```bash
python environments/durian_swe_env/durian_swe_env.py serve \
...
2. Create your env file inheriting from `DurianAgentBaseEnv`
3. Implement the four abstract methods + `evaluate()`

```python
from environments.durian_base_env import DurianAgentBaseEnv, DurianAgentEnvConfig

class MyEnvConfig(DurianAgentEnvConfig):
    pass  # Add custom fields as needed

class MyEnv(DurianAgentBaseEnv):
    name = "my-env"
...
2. Inherit from `DurianAgentBaseEnv`
...
|-------|-------------|---------|
| `enabled_toolsets` | Which durian toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `durian` |
