# OpenAI-Compatible API Server for Durian Agent
...
SSE streaming. By exposing this endpoint, durian-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  durian-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.durian/.env`
2. Run `durian gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with durian-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns durian-agent as a model) |
...
Authorization: Bearer durian-api-key-here
...
But durian-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "durian-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured durian-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of durian-agent's core prompt
...
Once implemented, durian-agent works as a drop-in backend for:
