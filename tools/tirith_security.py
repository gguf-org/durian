...
it is automatically downloaded from GitHub releases to $DURIAN_HOME/bin/tirith.
...
def _get_durian_home() -> str:
    """Return the Durian home directory, respecting DURIAN_HOME env var."""
    return str(get_durian_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_durian_home(), ".tirith-install-failed")
...
def _durian_bin_dir() -> str:
    """Return $DURIAN_HOME/bin, creating it if needed."""
    d = os.path.join(_get_durian_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $DURIAN_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_durian_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $DURIAN_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $DURIAN_HOME/bin/tirith
    ...
    durian_bin = os.path.join(_durian_bin_dir(), "tirith")
    if os.path.isfile(durian_bin) and os.access(durian_bin, os.X_OK):
        _resolved_path = durian_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return durian_bin
