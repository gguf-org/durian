...
- ``durian_cli.providers``        -- canonical provider identity + overlays
- ``durian_cli.model_normalize``  -- per-provider name formatting
...
from durian_cli.providers import (
...
_DURIAN_MODEL_WARNING = (
    "Nous Research Durian 3 & 4 models are NOT agentic and are not designed "
    "for use with Durian Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Durian 3 / Durian 4 chat families.
# The previous substring check (`"durian" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``durian-brain:qwen3-14b-ctx16k`` that just
# happen to carry "durian" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   gguf-org/Durian-3-Llama-3.1-70B, durian-4-405b, openrouter/durian3:70b
# Negative examples it must NOT match:
#   durian-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_DURIAN_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])durian[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_durian_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Durian 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_DURIAN_NON_AGENTIC_RE.search(model_name))


def _check_durian_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Durian 3/4 chat model."""
    if is_nous_durian_non_agentic(model_name):
        return _DURIAN_MODEL_WARNING
    return ""
...
def switch_model(
    ...
    from durian_cli.runtime_provider import resolve_runtime_provider
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'durian model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'durian doctor' — config issues detected:"
    ...
    durian_warn = _check_durian_model_warning(new_model)
    if durian_warn:
        warnings.append(durian_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from durian_cli/models.py (OPENROUTER_MODELS,
    ...
    from durian_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by durian provider ID
    ...
    # --- 1. Check Durian-mapped providers ---
    for durian_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(durian_id)
        ...
        model_ids = curated.get(durian_id, [])
        total = len(model_ids)
    ...
    # --- 2. Check Durian-only providers (nous, openai-codex, copilot, opencode-go) ---
    from durian_cli.providers import DURIAN_OVERLAYS
    from durian_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Durian provider ID.
    # DURIAN_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Durian IDs ("copilot").
    _mdev_to_durian = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in DURIAN_OVERLAYS.items():
        if pid in seen_slugs:
            continue

        # Resolve Durian slug — e.g. "github-copilot" → "copilot"
        durian_slug = _mdev_to_durian.get(pid, pid)
        if durian_slug in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                from durian_cli.auth import _load_auth_store
                store = _load_auth_store()
                providers_store = store.get("providers", {})
                pool_store = store.get("credential_pool", {})
                if store and (
                    pid in providers_store or durian_slug in providers_store
                    or pid in pool_store or durian_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(durian_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", durian_slug, exc)
        ...
        # configured.
        if not has_creds and durian_slug == "anthropic":
            try:
                from agent.anthropic_adapter import (
                    read_claude_code_credentials,
                    read_durian_oauth_credentials,
                )
                durian_creds = read_durian_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (durian_creds and durian_creds.get("accessToken")) or \
        ...
        # Use curated list — look up by Durian slug, fall back to overlay key
        model_ids = curated.get(durian_slug, []) or curated.get(pid, [])
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": durian_slug,
            "name": get_label(durian_slug),
            "is_current": durian_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "durian",
        ...
        seen_slugs.add(pid)
        seen_slugs.add(durian_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or DURIAN_OVERLAYS (keeps /model in sync
    # with `durian model`).
    try:
        from durian_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
