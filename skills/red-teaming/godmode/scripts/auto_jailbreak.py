...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.durian/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("DURIAN_HOME", Path.home() / ".durian")) / "skills" / "red-teaming" / "godmode"
...
DURIAN_HOME = Path(os.getenv("DURIAN_HOME", Path.home() / ".durian"))
CONFIG_PATH = DURIAN_HOME / "config.yaml"
PREFILL_PATH = DURIAN_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Durian models — already uncensored, just needs clean prompt
    "durian": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "durian" in model_lower or "nous" in model_lower:
        return "durian"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Durian config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.durian/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Durian for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Durian for changes to take effect.")
