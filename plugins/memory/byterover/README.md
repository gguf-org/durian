...
```bash
durian memory setup    # select "byterover"
```

Or manually:
```bash
durian config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.durian/.env
```
...
Working directory: `$DURIAN_HOME/byterover/` (profile-scoped).
