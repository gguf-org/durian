...
def _load_supermemory_config(durian_home: str) -> dict:
    config = _default_config()
    config_path = Path(durian_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, durian_home: str) -> None:
    config_path = Path(durian_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `durian memory setup`.
        # All other options are documented for $DURIAN_HOME/supermemory.json
    ...
    def save_config(self, values, durian_home):
        ...
        _save_supermemory_config(sanitized, durian_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from durian_constants import get_durian_home
        self._durian_home = kwargs.get("durian_home") or str(get_durian_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._durian_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "durian", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "durian_memory", "target": target, "type": "explicit_memory"},
