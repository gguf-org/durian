# nix/nixosModules.nix — NixOS module for durian
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: durian runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $DURIAN_HOME/.env
# and read by durian at startup — no container recreation needed for env changes.
#
# Tool resolution: the durian wrapper uses --suffix PATH for nix store tools,
...
#     environmentFiles = [ config.sops.secrets."durian/env".path ];
#   };
#
{ inputs, ... }: {
  ...
  let
    cfg = config.services.durian;
    durian = inputs.self.packages.${pkgs.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-durian)
    deepConfigType = lib.types.mkOptionType {
      name = "durian-config-attrs";
      description = "Durian YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "durian-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "durian-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'DURIAN_DOC_EOF'\n${value}\nDURIAN_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "durian";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/durian";
    ...
    # durian user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "durian-container-entrypoint" ''
      set -eu

      DURIAN_UID="''${DURIAN_UID:?DURIAN_UID must be set}"
      DURIAN_GID="''${DURIAN_GID:?DURIAN_GID must be set}"

      # ── Group: ensure a group with GID=$DURIAN_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$DURIAN_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="durian"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$DURIAN_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$DURIAN_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$DURIAN_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$DURIAN_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="durian"
        TARGET_HOME="/home/durian"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$DURIAN_UID" -g "$DURIAN_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$DURIAN_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$DURIAN_UID:$DURIAN_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure DURIAN_HOME is owned by the target user
      if [ -n "''${DURIAN_HOME:-}" ] && [ -d "$DURIAN_HOME" ]; then
        chown -R "$DURIAN_UID:$DURIAN_GID" "$DURIAN_HOME"
      fi
      ...
      if [ ! -f /var/lib/durian-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        echo "First boot: provisioning agent tools..."
        apt-get update -qq
        apt-get install -y -qq sudo nodejs npm curl
        touch /var/lib/durian-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/durian ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/durian
        chmod 0440 /etc/sudoers.d/durian
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$DURIAN_UID" --regid="$DURIAN_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $DURIAN_HOME/.env.
    ...
    # Default: /var/lib/durian/workspace → /data/workspace.
  ...
  in {
    options.services.durian = with lib; {
      enable = mkEnableOption "Durian Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = durian;
        description = "The durian package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/durian";
        description = "State directory. Contains .durian/ subdir (DURIAN_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Durian config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $DURIAN_HOME/.env at activation time.
          Durian reads this file on every startup via load_durian_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $DURIAN_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $DURIAN_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `durian gateway`.";
      };
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the durian CLI to environment.systemPackages and export
          DURIAN_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.durian symlink to the service
            stateDir. These users are automatically added to the durian group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.durian.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the durian CLI to system PATH and export DURIAN_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.durian/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ cfg.package ];
        environment.variables.DURIAN_HOME = "${cfg.stateDir}/.durian";
      })
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.durian: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed durian
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure durian is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.durian        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.durian/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.durian/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.durian/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.durian/memories 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."durian-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.durian
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.durian ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.durian ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.durian -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories; do
            mkdir -p "${cfg.stateDir}/.durian/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.durian/$_subdir"
            chmod 2770 "${cfg.stateDir}/.durian/$_subdir"
            find "${cfg.stateDir}/.durian/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.durian/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.durian/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.durian/config.yaml
            chmod 0640 ${cfg.stateDir}/.durian/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.durian/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.durian/.managed
          chmod 0644 ${cfg.stateDir}/.durian/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.durian/.container-mode <<'DURIAN_CONTAINER_MODE_EOF'
# Written by NixOS activation script. Do not edit manually.
backend=${cfg.container.backend}
container_name=${containerName}
exec_user=${cfg.user}
durian_bin=${containerDataDir}/current-package/bin/durian
DURIAN_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.durian/.container-mode
            chmod 0644 ${cfg.stateDir}/.durian/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.durian/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.durian";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.durian" ]; then
                  rm -f "${symlinkPath}"
                  echo "durian: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.durian -> stateDir/.durian for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.durian";
                target = "${cfg.stateDir}/.durian";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "durian: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.durian/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.durian/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.durian/auth.json
              fi
            ''}
          ''}
          ...
          # Durian reads $DURIAN_HOME/.env at startup via load_durian_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.durian/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'DURIAN_NIX_ENV_EOF'
${envFileContent}
DURIAN_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.durian = {
          description = "Durian Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            DURIAN_HOME = "${cfg.stateDir}/.durian";
            DURIAN_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $DURIAN_HOME/.env by the activation script. load_durian_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${cfg.package}/bin/durian"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the durian group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.durian = {
          description = "Durian Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              DURIAN_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              DURIAN_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env DURIAN_UID="$DURIAN_UID" \
                --env DURIAN_GID="$DURIAN_GID" \
                --env DURIAN_HOME=${containerDataDir}/.durian \
                --env DURIAN_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/durian gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
