"""durian webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    durian webhook subscribe <name> [options]
    durian webhook list
    durian webhook remove <name>
    durian webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.durian/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _durian_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: durian gateway run
"""
...
def webhook_command(args):
    """Entry point for 'durian webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: durian webhook {subscribe|list|remove|test}")
        print("Run 'durian webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (durian gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: durian webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from durian webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (durian gateway run)")
