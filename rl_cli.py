...
if tinker_atropos_dir.exists():
    ...
    os.environ['DURIAN_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to durian directory if submodule not found
...
from durian_constants import get_durian_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_durian_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.durian/config.yaml if not provided)
    ...
    """
    # Load config from ~/.durian/config.yaml
    config = load_durian_config()
