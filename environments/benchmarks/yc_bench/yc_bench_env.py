...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs DurianAgentLoop with terminal tool only
...
Requires: pip install durian[yc-bench]
"""
...
class YCBenchEvalConfig(DurianAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends DurianAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(DurianAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The DurianAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs DurianAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our DurianAgentLoop.
            ...
            # ----------------------------------------------------------
            # Step 2: Run the DurianAgentLoop
            # ----------------------------------------------------------
