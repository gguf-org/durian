...
```bash
durian memory setup    # select "hindsight"
```
...
```bash
durian config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.durian/.env
```
...
Durian spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.durian/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p durian ui start
```
...
Config file: `~/.durian/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `durian` | Memory bank name |
...
| `retain_context` | `conversation between Durian Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.durian/.env` as `HINDSIGHT_LLM_API_KEY`.
