...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      durian = inputs.self.packages.${system}.default;
      ...
      configKeys = pkgs.runCommand "durian-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${durianVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = lib.optionalAttrs pkgs.stdenv.hostPlatform.isLinux {
        ...
        package-contents = pkgs.runCommand "durian-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${durian}/bin/durian || (echo "FAIL: durian binary missing"; exit 1)
          test -x ${durian}/bin/durian || (echo "FAIL: durian binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${durian}/bin/durian version 2>&1 | grep -qi "durian" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "durian-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in durian durian durian-acp; do
            test -x ${durian}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "durian-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking durian --help ==="
          ${durian}/bin/durian --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${durian}/bin/durian --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "durian-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${durian}/share/durian/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${durian}/share/durian/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "DURIAN_BUNDLED_SKILLS" ${durian}/bin/durian || \
            (echo "FAIL: DURIAN_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: DURIAN_BUNDLED_SKILLS set in wrapper"
        ...
        # Verify DURIAN_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "durian-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(DURIAN_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking DURIAN_MANAGED guards ==="
          check_blocked "config set" ${durian}/bin/durian config set model foo
          check_blocked "config edit" ${durian}/bin/durian config edit
        ...
        in pkgs.runCommand "durian-config-roundtrip" {
          ...
          merge_and_load() {
            local durian_home="$1"
            export DURIAN_HOME="$durian_home"
            ${configMergeScript} ${nixSettings} "$durian_home/config.yaml"
            ${durianVenv}/bin/python3 -c '
import json, sys
