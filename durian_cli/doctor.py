"""
Doctor command for durian CLI.

Diagnoses issues with Durian Agent setup.
"""
...
from durian_cli.config import get_project_root, get_durian_home, get_env_path
from durian_constants import display_durian_home

PROJECT_ROOT = get_project_root()
DURIAN_HOME = get_durian_home()
_DHH = display_durian_home()  # user-facing display path (e.g. ~/.durian or ~/.durian/profiles/coder)

# Load environment variables from ~/.durian/.env so API key checks work
...
from durian_constants import OPENROUTER_MODELS_URL
...
from durian_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.durian/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `durian`.
    os.environ.setdefault("DURIAN_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 Durian Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'durian setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'durian setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'durian setup' to create one")
                issues.append("Run 'durian setup' to create .env")
    
    # Check ~/.durian/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = DURIAN_HOME / 'config.yaml'
    ...
    if config_path.exists():
        try:
            from durian_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'durian setup' to migrate config")
                else:
                    issues.append("Run 'durian doctor --fix' or 'durian setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'durian doctor --fix'")
        except Exception:
            pass
    ...
    if durian_home.exists():
        ...
        if should_fix:
            durian_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = durian_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Durian a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Durian Agent Persona\n\n"
                "<!-- Edit this file to customize how Durian communicates. -->\n\n"
                "You are Durian, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = durian_home / "memories"
    ...
    state_db_path = durian_home / "state.db"
    ...
    wal_path = durian_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'durian doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (durian bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "durian"
        ...
        _cmd_link = _cmd_link_dir / "durian"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(durian not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/durian → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/durian points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/durian → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/durian — run 'durian doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/durian exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/durian not found",
                    "(durian command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/durian → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/durian symlink — run 'durian doctor --fix'")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'durian setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: durian skills list)")
    ...
    try:
        ...
        _mem_cfg_path = DURIAN_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: durian memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: durian memory setup")
                issues.append("No Honcho API key — run 'durian memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run durian memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: durian memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: durian memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from durian_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "durian -p" in content:
                            _m = _re.search(r"durian -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'durian doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
