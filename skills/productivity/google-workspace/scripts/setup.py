#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Durian Agent.
...
try:
    from durian_constants import display_durian_home, get_durian_home
except ModuleNotFoundError:
    DURIAN_AGENT_ROOT = Path(__file__).resolve().parents[4]
    if DURIAN_AGENT_ROOT.exists():
        sys.path.insert(0, str(DURIAN_AGENT_ROOT))
    from durian_constants import display_durian_home, get_durian_home

DURIAN_HOME = get_durian_home()
TOKEN_PATH = DURIAN_HOME / "google_token.json"
CLIENT_SECRET_PATH = DURIAN_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = DURIAN_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Durian profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Durian home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_durian_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Durian")
