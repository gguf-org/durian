...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("DURIAN_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $DURIAN_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_DURIAN_ENV_PATH = (
    r'(?:~\/\.durian/|'
    r'(?:\$home|\$\{home\})/\.durian/|'
    r'(?:\$durian_home|\$\{durian_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bdurian\s+gateway\s+(stop|restart)\b', "stop/restart durian gateway (kills running agents)"),
    (r'\bdurian\s+update\b', "durian update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart durian-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart durian-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(durian|gateway|cli\.py)\b', "kill durian/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill durian` but not
    # `kill -9 $(pgrep -f durian)` because the substitution is opaque
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["DURIAN_SPINNER_PAUSE"] = "1"
    try:
        while True:
            print()
        ...
        if "DURIAN_SPINNER_PAUSE" in os.environ:
            del os.environ["DURIAN_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("DURIAN_YOLO_MODE") or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("DURIAN_INTERACTIVE")
    is_gateway = os.getenv("DURIAN_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("DURIAN_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("DURIAN_YOLO_MODE") or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("DURIAN_INTERACTIVE")
    is_gateway = os.getenv("DURIAN_GATEWAY_SESSION")
    is_ask = os.getenv("DURIAN_EXEC_ASK")
