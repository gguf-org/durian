...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/durian>
...
```bash
durian honcho setup    # full interactive wizard (cloud or local)
durian memory setup    # generic picker, also works
```

Or manually:
```bash
durian config set memory.provider honcho
echo "HONCHO_API_KEY=your-key" >> ~/.durian/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$DURIAN_HOME/honcho.json` | Profile-local (isolated Durian instances) |
| 2 | `~/.durian/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Durian profile: `durian` (default) or `durian.<profile>`.
...
Host key derivation: `DURIAN_HONCHO_HOST` env > active profile (`durian.<profile>`) > `"durian"`.
...
| `DURIAN_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `durian honcho setup` | Full interactive setup wizard |
| `durian honcho status` | Show resolved config for active profile |
| `durian honcho enable` / `disable` | Toggle Honcho for active profile |
| `durian honcho mode <mode>` | Change recall or observation mode |
| `durian honcho peer --user <name>` | Update user peer name |
| `durian honcho peer --ai <name>` | Update AI peer name |
| `durian honcho tokens --context <N>` | Set context token budget |
| `durian honcho tokens --dialectic <N>` | Set dialectic max chars |
| `durian honcho map <name>` | Map current directory to a session name |
| `durian honcho sync` | Create host blocks for all Durian profiles |
