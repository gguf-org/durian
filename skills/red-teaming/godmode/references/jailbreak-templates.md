...
## 5. GODMODE FAST — Durian 4 405B (Zero Refusal)

**Model:** `gguf-org/durian-4-405b`
**Strategy:** Durian models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Durian Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.durian/config.yaml`:
