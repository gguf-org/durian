...
# 5. Symlinks the 'durian' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}𖤓 Durian Agent Setup${NC}"
echo ""
...
if [ -d "tinker-atropos" ] && [ -f "tinker-atropos/pyproject.toml" ]; then
    echo -e "${CYAN}→${NC} tinker-atropos submodule found — skipping install (optional, for RL training)"
    echo -e "${CYAN}→${NC}   To install: $UV_CMD pip install -e \"./tinker-atropos\""
fi
...
# ============================================================================
# PATH setup — symlink durian into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up durian command..."

DURIAN_BIN="$SCRIPT_DIR/venv/bin/durian"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$DURIAN_BIN" "$COMMAND_LINK_DIR/durian"
echo -e "${GREEN}✓${NC} Symlinked durian → $COMMAND_LINK_DISPLAY_DIR/durian"
...
# ============================================================================
# Seed bundled skills into ~/.durian/skills/
# ============================================================================

DURIAN_SKILLS_DIR="${DURIAN_HOME:-$HOME/.durian}/skills"
mkdir -p "$DURIAN_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.durian/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$DURIAN_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  durian status        # Check configuration"
if is_termux; then
    echo "  durian gateway       # Run gateway in foreground"
else
    echo "  durian gateway install # Install gateway service (messaging + cron)"
fi
echo "  durian cron list     # View scheduled jobs"
echo "  durian doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m durian_cli.main setup
fi
