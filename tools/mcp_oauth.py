...
This module provides the glue:
    - ``DurianTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Durian Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses DURIAN_HOME so each profile gets its own OAuth tokens.
    Layout: ``DURIAN_HOME/mcp-tokens/``
    """
    try:
        from durian_constants import get_durian_home
        base = Path(get_durian_home())
    except ImportError:
        base = Path(os.environ.get("DURIAN_HOME", str(Path.home() / ".durian")))
...
# ---------------------------------------------------------------------------
# DurianTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class DurianTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        DURIAN_HOME/mcp-tokens/<server_name>.json         -- tokens
        DURIAN_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Durian.</p></body></html>"
            ) if code else (
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = DurianTokenStorage(server_name)
    storage.remove()
...
def build_oauth_auth(
    ...
    # --- Client metadata ---
    client_name = cfg.get("client_name", "Durian Agent")
    scope = cfg.get("scope")
