"""
Interactive setup wizard for Durian Agent.
...
Config files are stored in ~/.durian/ for easy access.
"""
...
_DOCS_BASE = "https://durian.gguf-org.com/docs"
...
# display_durian_home imported lazily at call sites (stale-module safety during durian update)

from durian_cli.colors import Colors, color
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print_info("Configure Durian using environment variables or config commands:")
    print_info("  durian config set model.provider custom")
    print_info("  durian config set model.base_url http://localhost:8080/v1")
    print_info("  durian config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'durian setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0) -> int:
    ...
    from durian_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from durian_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'durian setup')")


def _print_setup_summary(config: dict, durian_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'durian setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'durian setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'durian setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'durian setup tools' to configure them,"
        )
        from durian_constants import display_durian_home as _dhh
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {durian_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('durian setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('durian setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('durian setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('durian setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('durian setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('durian config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('durian config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('durian config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('durian', Colors.GREEN)}              Start chatting")
    print(f"   {color('durian gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('durian doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `durian tools` and `durian setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``durian model``)
    ...
    provider added to ``durian model`` is automatically available here.
    ...
    # Delegate to the shared durian model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from durian_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: durian model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from durian_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Durian can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Durian can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'durian setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.durian/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'durian setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Durian runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Durian via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("DURIAN_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `durian setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("DURIAN_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("DURIAN_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Durian delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://durian.gguf-org.com/docs/user-guide/messaging/slack/")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Durian delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Durian delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'durian whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'durian whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from durian_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from durian_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from durian_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from durian_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from durian_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from durian_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from durian_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from durian_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_qqbot():
    ...
    print_info("Connects Durian to QQ via the Official QQ Bot API (v2).")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Durian to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from durian_cli.gateway import _PLATFORMS
    qq_platform = next((p for p in _PLATFORMS if p["key"] == "qqbot"), None)
    if qq_platform:
        from durian_cli.gateway import _setup_standard_platform
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://durian.gguf-org.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://durian.gguf-org.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  durian config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Durian from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'durian setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     durian config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: durian gateway install")
            else:
                print_info("  You can install later: durian gateway install")
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo durian gateway install --system")
                print_info("  Or run in foreground:  durian gateway")
        else:
            from durian_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   durian gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   durian gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `durian setup tools` and `durian tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from durian_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from durian_cli.auth import get_active_provider
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_durian.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_durian migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_durian", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Durian.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Durian to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Durian at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Durian at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Durian at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Durian at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Durian equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Durian config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Durian.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Durian's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(durian_home: Path) -> bool:
    ...
    print_info("Durian can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: durian claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=durian_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: durian claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Durian configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Durian config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Durian (use durian claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      durian setup           — full or quick (auto-detected)
      durian setup model     — just model/provider
      durian setup tts       — just text-to-speech
      durian setup terminal  — just terminal backend
      durian setup gateway   — just messaging platforms
      durian setup tools     — just tool configuration
      durian setup agent     — just agent settings
    """
    ...
    print(
        color(
            "│  Let's configure your Durian Agent installation.        │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Durian configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, durian_home)
            return
        elif choice == 1:
            # Full setup — fall through to run all sections
            pass
        elif choice == 7:
            print_info("Exiting. Run 'durian setup' again when ready.")
            return
        elif 2 <= choice <= 6:
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, durian_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        setup_mode = prompt_choice("How would you like to set up Durian?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, durian_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {durian_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'durian config edit'")
...
def _run_first_time_quick_setup(config: dict, durian_home, is_existing: bool):
    ...
    print()
    print_info("  Configure all settings:    durian setup")
    print_info("  Connect Telegram/Discord:  durian setup gateway")
    print()
...
def _run_quick_setup(config: dict, durian_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'durian setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Durian to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'durian setup gateway'.")
